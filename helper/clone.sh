...
function repositories() {
  ...
  echo -e "  Getting Repository Info\n"
  sleep 2s

  echo -e "${dark}  ──────────────────────────────────────────────────────────────${reset}"
  echo -e "  ${gray}  Repositories${reset}"
  echo -e "${dark}  ──────────────────────────────────────────────────────────────${reset}"
  printf "    ${dark}%-40s  %-10s${reset}\n" "Repository" "Size"
  echo -e "${dark}  ──────────────────────────────────────────────────────────────${reset}\n"
  ...
  for REPOSITORY_API in "${REPOSITORY_APIS[@]}"; do
    ...
    printf "    ${COLOR_SUCCESS}%-40s${COLOR_BASED}  ${COLOR_WARNING}%8s${COLOR_BASED}\n" $REPOSITORY_NAME $(repoSize $REPOSITORY_API)
  done
